// SkillTreeUpgradeManager.cs

using UnityEngine;
using TMPro;
using System.Collections.Generic;

public class SkillTreeUpgradeManager : MonoBehaviour
{
    public static SkillTreeUpgradeManager Instance;
    ...
    private SkillTreeConnectionUI[] connections;
    ...
    void Start()
    {
        ...
        connections = Object.FindObjectsByType<SkillTreeConnectionUI>(
            FindObjectsInactive.Include,
            FindObjectsSortMode.None
        );
    ...
    {
        CurrencyManager activeCurrencyManager = currencyManager != null
            ? currencyManager
            : CurrencyManager.Instance;

        if (activeCurrencyManager != null && currencyText != null)
            currencyText.text = $"C: {activeCurrencyManager.GetCurrentCurrency()}";
    }
    ...
    public void ShowInfo(UpgradeData data)
    {
        ...
        costText.text = $"Cost: {GetUpgradeCost(data)}";
    }
    ...
    public int GetUpgradeCost(UpgradeData data)
    {
        if (data == null)
            return 0;
    ...
    public bool CanAfford(UpgradeData data)
    {
        CurrencyManager activeCurrencyManager = currencyManager != null
            ? currencyManager
            : CurrencyManager.Instance;

        if (activeCurrencyManager == null)
            return false;

        return activeCurrencyManager.GetCurrentCurrency() >= GetUpgradeCost(data);
    }

    public bool IsPurchasable(UpgradeData data)
    {
        if (data == null)
            return false;
        ...
        if (IsGroupLockedFor(data))
            return false;

        return CanAfford(data);
    }
    ...
    public void TryBuy(UpgradeNode node)
    {
        ...
        if (activeCurrencyManager == null)
        {
            Debug.Log("Missing currency manager");
            return;
        }

        int finalCost = GetUpgradeCost(data);
        if (!activeCurrencyManager.SpendCurrency(finalCost))
        {
    ...
    private void SubscribeToCurrencyChanges()
    {
        ...
        if (activeCurrencyManager != null)
            activeCurrencyManager.OnCurrencyChanged += HandleCurrencyChanged;
    }
